...
BENCHMARK(BM_sorted_range_insertion<flat_map::flat_map<int, int>, k_factor>)
    ->Ranges({range, range});
BENCHMARK(BM_sorted_range_insertion<
              flat_map::flat_map<int, int, std::less<int>, std::deque<std::pair<int, int>>>,
              k_factor>)
    ->Ranges({range, range});
