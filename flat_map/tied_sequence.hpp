...
class zip_iterator {
   ...
   public:
    ...
    constexpr auto get_iterator() const noexcept {
   ...
   public:
    constexpr reference operator*() const {
        return _dereference(std::index_sequence_for<Iterators...>{});
    }

    // XXX: operator-> requires returning "raw pointer" or call op-> recursively
    // until getting "raw pointer". constexpr pointer operator->() const;
    ...
    constexpr difference_type operator-(zip_iterator const& other) const {
        return std::get<0>(_it) - std::get<0>(other._it);
    }

    friend constexpr void swap(zip_iterator& lhs, zip_iterator& rhs) noexcept(
        noexcept(std::swap(lhs, rhs))
    ) {
        std::swap(lhs._it, rhs._it);
    }

    friend constexpr void iter_swap(zip_iterator const& lhs, zip_iterator const& rhs) noexcept(
        (std::is_nothrow_swappable_v<typename std::iterator_traits<Iterators>> && ...)
    ) {
        tuple_transform(
            [](auto& l, auto& r) {
                using std::swap;
...
constexpr auto get_iterator(const zip_iterator<Iterators...>& zip_it) noexcept {
    return zip_it.template get_iterator<N>();
}

template <typename... Iterators>
constexpr auto operator+(
    zip_iterator<Iterators...> lhs, typename zip_iterator<Iterators...>::difference_type n
) {
    return lhs += n;
}
...
template <typename... Iterators>
constexpr auto operator+(
    typename zip_iterator<Iterators...>::difference_type n, zip_iterator<Iterators...> rhs
) {
    return rhs += n;
}
...
class unzip_iterator {
   ...
   public:
    ...
    using pointer =
        detail::copy_cv_t<value_type, typename std::iterator_traits<Iterator>::pointer>*;
    // using reference = detail::copy_cv_t<value_type,
   ...
   public:
    ...
    constexpr unzip_iterator& operator++() {
        ++_base;
        return *this;
    }
    ...
    constexpr unzip_iterator& operator+=(difference_type n) {
        _base += n;
        return *this;
    }
    constexpr unzip_iterator& operator-=(difference_type n) {
    ...
    constexpr difference_type operator-(unzip_iterator const& other) const {
        return _base - other._base;
    }

    friend constexpr void swap(
        unzip_iterator& lhs, unzip_iterator& rhs
    ) noexcept(std::is_nothrow_swappable_v<Iterator>) {
        using std::swap;
        return swap(lhs._base, rhs._base);
    }

    friend constexpr bool operator==(unzip_iterator const& lhs, unzip_iterator const& rhs) {
...
template <std::size_t N, typename Iterator>
constexpr auto unzip(Iterator itr) {
    return unzip_iterator<N, Iterator>{itr};
}

template <std::size_t N, typename Iterator>
constexpr auto operator+(
    unzip_iterator<N, Iterator> lhs, typename unzip_iterator<N, Iterator>::difference_type n
) {
    return lhs += n;
}
...
template <std::size_t N, typename Iterator>
constexpr auto operator+(
    typename unzip_iterator<N, Iterator>::difference_type n, unzip_iterator<N, Iterator> rhs
) {
    return rhs += n;
}
...
template <std::size_t N, typename Iterator>
constexpr bool operator!=(
    unzip_iterator<N, Iterator> const& lhs, unzip_iterator<N, Iterator> const& rhs
) {
    return !(lhs == rhs);
}
...
class tied_sequence {
   ...
   public:
    ...
    using allocator_type =
        detail::fake_allocator<std::tuple<typename Sequences::allocator_type...>>;
   ...
   private:
    ...
    constexpr tied_sequence(
        std::index_sequence<N...>, size_type count, T&& value, allocator_type const& alloc
    )
    ...
    constexpr tied_sequence(
        ...
        size_type count,
        T&&       value,
        typename Sequences::allocator_type const&... alloc
    )
        : _seq{Sequences(count, std::get<N>(std::forward<T>(value)), alloc)...} {}

    template <std::size_t... N, typename T>
    constexpr tied_sequence(
        std::index_sequence<N...>, T&& value, typename Sequences::allocator_type const&... alloc
    )
    ...
    constexpr tied_sequence(
        ...
        InputIterator,
        InputIterator,
        typename Sequences::allocator_type const&...
    ) {
        // TODO
   ...
   public:
    ...
    constexpr tied_sequence(
        size_type count, value_type const& value, typename Sequences::allocator_type const&... alloc
    )
        : tied_sequence(std::index_sequence_for<Sequences...>{}, count, value, alloc...) {}

    constexpr tied_sequence(size_type count)
        : tied_sequence(count, typename Sequences::allocator_type{}...) {}
    ...
    template <typename InputIterator>
    constexpr tied_sequence(
        InputIterator first, InputIterator last, typename Sequences::allocator_type const&... alloc
    )
        : tied_sequence(
    ...
    constexpr tied_sequence(
        tied_sequence const& other, typename Sequences::allocator_type const&... alloc
    )
        : tied_sequence(std::index_sequence_for<Sequences...>{}, other._seq, alloc...) {}
    ...
    constexpr tied_sequence(
        tied_sequence&& other, typename Sequences::allocator_type const&... alloc
    )
        : tied_sequence(std::index_sequence_for<Sequences...>{}, std::move(other._seq), alloc...) {}
    ...
    constexpr tied_sequence(
        std::initializer_list<value_type> init, typename Sequences::allocator_type const&... alloc
    )
    ...
    constexpr tied_sequence& operator=(tied_sequence&& other) noexcept(
        ((std::allocator_traits<
              typename Sequences::allocator_type>::propagate_on_container_move_assignment::value
    ...
    constexpr void assign(size_type count, value_type const& value) {
        detail::tuple_transform(
            [count](auto& c, auto& value) { c.assign(count, value); },
            _seq,
            value
        );
    }
   ...
   private:
    template <typename InputIterator, std::size_t... N>
    constexpr void _assign(
        InputIterator, InputIterator, std::index_sequence<N...>, std::input_iterator_tag
    ) {
        // TODO
        static_assert(sizeof(InputIterator) && false, "not implemented yet");
    }
   ...
   public:
    ...
    constexpr allocator_type get_allocator() const noexcept {
        return std::apply(
            [](auto&... seqs) {
                return allocator_type{std::allocator_arg, seqs.get_allocator()...};
            },
            _seq
        );
    }
    ...
    constexpr reference at(size_type pos) {
        if (!(pos < size())) {
            throw std::out_of_range{"tied_sequence::at"};
        }
        return operator[](pos);
    }

    constexpr const_reference at(size_type pos) const {
        return const_cast<tied_sequence*>(this)->at(pos);
    }

    constexpr reference operator[](size_type pos) { return *std::next(begin(), pos); }

    constexpr const_reference operator[](size_type pos) const {
        return const_cast<tied_sequence*>(this)->operator[](pos);
    }
    ...
    constexpr iterator begin() noexcept {
        return detail::tuple_transform([](auto& c) { return c.begin(); }, _seq);
    }
    ...
    constexpr iterator       end() noexcept {
              return detail::tuple_transform([](auto& c) { return c.end(); }, _seq);
    }
   ...
   public:
    constexpr size_t max_size() const noexcept {
        return detail::tuple_reduction(
            [](auto&&... c) { return std::min({c.max_size()...}); },
            _seq
        );
    }

    constexpr void clear() noexcept {
        detail::tuple_reduction([](auto&... c) { (c.clear(), ...); }, _seq);
    }

    constexpr iterator insert(const_iterator pos, value_type const& value) {
        return detail::tuple_transform(
            [](auto& c, auto pos, auto& value) { return c.insert(pos, value); },
            _seq,
            pos._it,
            value
        );
    ...
    constexpr iterator insert(const_iterator pos, value_type&& value) {
        return detail::tuple_transform(
            [](auto& c, auto pos, auto&& value) { return c.insert(pos, std::move(value)); },
            _seq,
            pos._it,
            std::move(value)
        );
    ...
    constexpr iterator insert(const_iterator pos, size_type count, value_type const& value) {
        return detail::tuple_transform(
            [count](auto& c, auto pos, auto& value) { return c.insert(pos, count, value); },
            _seq,
            pos._it,
            value
        );
   ...
   private:
    ...
    constexpr auto _emplace_helper(
        Container& c, Iterator pos, std::index_sequence<N...>, std::tuple<Args...> const& args
    ) {
    ...
    template <typename Container, typename Iterator, std::size_t... N, typename... Args>
    constexpr auto _emplace_helper(
        Container& c, Iterator pos, std::index_sequence<N...>, std::tuple<Args...>&& args
    ) {
        return c.emplace(pos, std::forward<Args>(std::get<N>(args))...);
    }
   ...
   public:
    ...
    constexpr iterator emplace(const_iterator pos, std::piecewise_construct_t, Args&&... args) {
        auto inserter = [this](auto& c, auto pos, auto&& args) {
            using indices_t =
                std::make_index_sequence<std::tuple_size_v<detail::remove_cvref_t<decltype(args)>>>;
    ...
    constexpr iterator erase(const_iterator pos) {
        return detail::tuple_transform(
            [](auto& c, auto pos) { return c.erase(pos); },
            _seq,
            pos._it
        );
    }
    ...
    constexpr iterator erase(const_iterator first, const_iterator last) {
        return detail::tuple_transform(
            [](auto& c, auto first, auto last) { return c.erase(first, last); },
            _seq,
            first._it,
            last._it
        );
    ...
    constexpr void pop_back() {
        detail::tuple_reduction([](auto&... c) { (c.pop_back(), ...); }, _seq);
    }

    constexpr void swap(tied_sequence& other) noexcept(
        ((std::allocator_traits<
              typename Sequences::allocator_type>::propagate_on_container_swap::value
          || std::allocator_traits<typename Sequences::allocator_type>::is_always_equal::value)
         && ...)
    ) {
        detail::tuple_transform(
    ...
    constexpr std::tuple<Sequences...> extract() && { return std::move(_seq); }

    constexpr void replace(std::tuple<Sequences...>&& seq) {
        // Calculate the cap and cup of the bit pattern representing the size of
        // each sequence.
        if (detail::tuple_reduction([](auto&... c) { return (c.size() & ...); }, seq)
            != detail::tuple_reduction([](auto&... c) { return (c.size() | ...); }, seq)) {
    ...
    constexpr const auto& get_sequence() const noexcept {
        return std::get<N>(_seq);
    }
};

template <std::size_t N, typename... Sequences>
constexpr const auto& get_sequence(const tied_sequence<Sequences...>& tied_seq) noexcept {
    return tied_seq.template get_sequence<N>();
}

template <typename... Sequences>
constexpr bool operator==(
    tied_sequence<Sequences...> const& lhs, tied_sequence<Sequences...> const& rhs
) {
...
constexpr typename tied_sequence<Sequences...>::size_type erase(
    tied_sequence<Sequences...>& c, U const& value
) {
...
constexpr typename tied_sequence<Sequences...>::size_type erase_if(
    tied_sequence<Sequences...>& c, Pred pred
) {
