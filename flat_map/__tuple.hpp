...
constexpr auto tuple_transform(F f, Head&& head, Tail&&... tail) {
    static_assert(
        ((std::tuple_size_v<remove_cvref_t<Head>> == std::tuple_size_v<remove_cvref_t<Tail>>)&&...)
    );
...
class tuple : public std::tuple<T...> {
   public:
    ...
    template <
        typename... U,
        typename =
            std::enable_if_t<std::is_constructible_v<std::tuple<T...>, std::tuple<U...> const&>>>
    ...
    template <
        ...
        typename = std::enable_if_t<std::is_assignable_v<
            decltype(std::declval<tuple<U...>&>().base()),
            std::tuple<T...> const&>>,
    ...
    template <
        typename... U,
        typename = std::enable_if_t<!std::is_assignable_v<
            decltype(std::declval<tuple<U...> const&>().base()),
            std::tuple<T...> const&>>,
        typename = std::enable_if_t<sizeof...(T) == sizeof...(U)>>
...
template <typename... T>
struct tuple_size<flat_map::detail::tuple<T...>> : public tuple_size<std::tuple<T...>> {};

template <std::size_t I, typename... T>
struct tuple_element<I, flat_map::detail::tuple<T...>> : public tuple_element<I, std::tuple<T...>> {
};
