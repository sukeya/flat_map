...
class flat_map
    : private detail::
          _flat_tree_base<flat_map<Key, T, Compare, Container>, Key, Compare, Container> {
   ...
   public:
    ...
    explicit flat_map(allocator_type const& alloc) : _super{alloc} {}
    ...
    template <typename InputIterator>
    flat_map(InputIterator first, InputIterator last, allocator_type const& alloc) : _super{alloc} {
        this->_initialize_container(first, last);
    }

    flat_map(flat_map const& other) = default;
    flat_map(flat_map const& other, allocator_type const& alloc) : _super{other, alloc} {}

    flat_map(flat_map&& other) = default;
    flat_map(flat_map&& other, allocator_type const& alloc) : _super{std::move(other), alloc} {}
    ...
    flat_map(std::initializer_list<value_type> init, allocator_type const& alloc) : _super{alloc} {
        this->_initialize_container(init.begin(), init.end());
    }

    explicit flat_map(range_order order, Container const& cont) : _super{order, cont} {}
    ...
    explicit flat_map(range_order order, Container&& cont) : _super{order, std::move(cont)} {}
   ...
   private:
    ...
    std::pair<iterator, bool> _insert_or_assign(K&& key, M&& obj) {
        ...
        if (!found) {
            itr = this->_container.emplace(itr, std::forward<K>(key), std::forward<M>(obj));
        } else {
            std::get<1>(*itr) = std::forward<M>(obj);
        }
        return {itr, !found};
    }
   ...
   public:
    ...
    iterator try_emplace(const_iterator hint, key_type const& key, Args&&... args) {
        return _try_emplace(hint, key, std::forward<Args>(args)...);
    }

    template <typename... Args>
    iterator try_emplace(const_iterator hint, key_type&& key, Args&&... args) {
        return _try_emplace(hint, std::move(key), std::forward<Args>(args)...);
    }

    using _super::erase;

    void swap(flat_map& other) noexcept(noexcept(this->_super::swap(other))) {
        _super::swap(other);
    }
...
bool operator==(
    flat_map<Key, T, Compare, Container> const& lhs, flat_map<Key, T, Compare, Container> const& rhs
) {
...
template <typename Key, typename T, typename Compare, typename Container>
void swap(
    flat_map<Key, T, Compare, Container>& lhs, flat_map<Key, T, Compare, Container>& rhs
) noexcept(noexcept(lhs.swap(rhs))) {
    lhs.swap(rhs);
}

template <typename Key, typename T, typename Compare, typename Container, typename Pred>
constexpr typename flat_map<Key, T, Compare, Container>::size_type erase_if(
    flat_map<Key, T, Compare, Container>& c, Pred pred
) {
...
flat_map(InputIterator, InputIterator, Compare = Compare(), Allocator = Allocator()) -> flat_map<
...
template <
    ...
    typename Allocator,
    typename = std::enable_if_t<detail::is_allocator_v<Allocator>>>
flat_map(InputIterator, InputIterator, Allocator) -> flat_map<
    detail::iter_key_t<InputIterator>,
    detail::iter_val_t<InputIterator>,
