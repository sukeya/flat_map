...
#define FLAT_MAP_DEFINE_CONCEPT(Name, T, Params, ...)                                            \
    ...
    struct Name##_body<R(Args...)> {                                                             \
        template <typename T>                                                                    \
        static constexpr auto check_impl Params->decltype(__VA_ARGS__);                          \
        template <typename D = R>                                                                \
        static constexpr detail::bool_t<decltype(check_impl<D>(std::declval<Args>()...))> check( \
            int                                                                                  \
        ) {                                                                                      \
            return true;                                                                         \
        }                                                                                        \
    ...
    template <typename T>                                                                        \
    inline constexpr bool Name = Name##_body<T Params>::check(0)
